...
while(1){
    ...
    if(strlen($tmp_str)<10){
        ...
        if(strpos($ttt,"=") === false  || strpos($tmp_str,"=")!==false) {
            echo "[!!] Magic:\n ------------------------------------------\n " . quoted_printable_encode($tmp_str) . "\n";
            if(file_exists("/tmp/a/".quoted_printable_encode($tmp_str))){
                $size = strlen(file_get_contents("/tmp/a/".quoted_printable_encode($tmp_str)));
                if($size>strlen("php://filter/" . $op_all.(($op_all == "")?'':'|').$op . '/resource=' . $input)){
                    file_put_contents("/tmp/a/" . quoted_printable_encode($tmp_str), "php://filter/" . $op_all.(($op_all == "")?'':'|').$op . '/resource=' . $input);
                }
            }
            else{
                file_put_contents("/tmp/a/" . quoted_printable_encode($tmp_str), "php://filter/" . $op_all.(($op_all == "")?'':'|').$op . '/resource=' . $input);
